...
// ==================== CARDLINK PAYMENT API ====================

// API для создания платежа через Cardlink
app.post('/api/payment/cardlink/create', async (req, res) => {
    try {
        const { orderId, amount, description, customerEmail, customerName } = req.body;
        
        if (!CARDLINK_API_TOKEN || !CARDLINK_SHOP_ID) {
            return res.status(500).json({ error: 'Cardlink не настроен. Добавьте CARDLINK_API_TOKEN и CARDLINK_SHOP_ID в .env' });
        }
        ...
        formData.append('description', description || `Заказ #${orderId}`);
        ...
        console.log('📤 Создание платежа Cardlink:', {
        ...
        db.prepare('UPDATE orders SET status = ? WHERE order_id = ?').run('awaiting_payment', orderId);
...
// Chargeback URL - обработка чарджбэка
...
app.post('/api/fix-reviews', async (req, res) => {
    try {
        const alekseyText = 'Все супер 👍 Купил со скидкой по промокоду, да еще и пробный период 30 дней итого 2 месяца подписки за 2К. Ребята отзывчивые, все объяснили, подсказали. Буду обращаться ещё. Лучше и искать нечего. Спасибо большое ребятам из Trust Store, удачи и процветания вам, при сегодняшних реалиях вы делаете больше дело.';
        
        // Удаляем старый отзыв от Aleksey T если есть (по telegram_comment_id или имени)
        const deletedAleksey = db.prepare(`DELETE FROM telegram_reviews WHERE author_name = 'Aleksey T' OR telegram_comment_id = 1273`).run();
        
        // Добавляем новый отзыв с текущей датой (чтобы он был первым в списке)
        const nowTimestamp = Math.floor(Date.now() / 1000);
        db.prepare(`
            INSERT INTO telegram_reviews 
            (telegram_user_id, author_name, review_text, rating, telegram_comment_id, telegram_date, created_at)
            VALUES (0, 'Aleksey T', ?, 5, 1273, ?, CURRENT_TIMESTAMP)
        `).run(alekseyText, nowTimestamp);
        
        console.log(`✅ Отзыв от Aleksey T добавлен (удалено старых: ${deletedAleksey.changes})`);
...
// ==================== CARDLINK PAYMENT ====================
// Cardlink настройки
const CARDLINK_API_TOKEN = process.env.CARDLINK_API_TOKEN || 'h0MpLJQPQcFdt1ZARaWE7zPys6SrLPoFTZVBW2hM'; // API токен из личного кабинета Cardlink
const CARDLINK_SHOP_ID = process.env.CARDLINK_SHOP_ID || '9PmRwbY25J'; // Shop ID из личного кабинета Cardlink
...
const SBP_PHONE = process.env.SBP_PHONE || '+79026833297'; // Озон Банк, Виктор С
const SBP_BANK = process.env.SBP_BANK || 'Озон Банк';
const SBP_NAME = process.env.SBP_NAME || 'Виктор С';
